...
def get_args():
    ...
    parser.add_argument(
        "contents_path",
        type=str,
        default="/home/jovyan/active-projects/macro-economics-textbook/contents/",
        help="The path to the top level of the contents directory tree",
    )
    parser.add_argument(
        "output_path",
        type=str,
        default="./subsections.csv",
...
def diff(text, clean_text):
    ...
    clean_text_lines = [
        s.strip() for s in clean_text.splitlines() if s.strip()
    ]  # delete empty lines
...
def clean_raw_text(subsection_text_mdx):
    ...
    subsection_text_html = markdown.markdown(
        subsection_text_md, extensions=["extra", "tables"]
    )
    subsection_text_html = html_table_pattern.sub(
        "", subsection_text_html
    )  # this gets any markdown tables now been converted to HTML
    return (
        BeautifulSoup(subsection_text_html, features="html.parser").get_text().strip()
    )
...
def generate_slugs(heading_list):
    ...
    slug_list1 = [
        heading.lower()
        .replace("-", " ")
        .translate(str.maketrans("", "", remove_these))
        .replace(" ", "-")
...
if __name__ == "__main__":
    ...
    for path in sorted(CONTENTS.glob("**/index.mdx")):
        rel_path = path.relative_to(
            "/home/jovyan/active-projects/macro-economics-textbook/contents/"
        )
        parents = reversed(
            rel_path.parents[:-1]
        )  # get all parent directories before '../contents'. We omit ([:-1]) the top-level directory
        module = next(parents).name.split("-")[1]  # all .mdx files belong to a module
        chapter = next(parents, Path("-0")).name.split("-")[
            1
        ]  # if the iterator is exhausted, this will make the chapter number 0
        section = next(parents, Path("-0")).name.split("-")[
            1
        ]  # if the iterator is exhausted, this will make the section number 0
        if verbose:
            print(
                f"{path.as_posix():<60}Module {module}, Chapter {chapter}, Section {section}"
            )
        mdx_sections.append(
            {
                "module": module,
    ...
    pattern = re.compile(
        r"(?:^#{1,2} )(.*?)$\s*(.*?)(?=\s*^#|\Z)", re.DOTALL | re.MULTILINE
    )
    ...
    for section in mdx_sections:
        ...
        for i, match in enumerate(matches):
            ...
            if (
                subsection_title.lower().strip() in subsections_to_skip
            ):  # lowercase() and strip() because formating is inconsistent
                if (
                    verbose and len(subsection_text) > 10
                ):  # if verbose, print the longer sections that we will be EXcluding
                    print("-" * 80)
                    print(subsection_title, "\n", subsection_text)
                    print("_" * 80)
                continue
            elif (
                verbose and len(subsection_text) < 100
            ):  # if verbose, print the shorter sections that we will still be INcluding
                print("-" * 80)
                print(subsection_title, "\n", subsection_text)
                print("_" * 80)
            else:
    ...
    # HTML cleanup
    exclamation_links_pattern = re.compile(
        r"!\["
    )  # Removes the exclamation point in links ![link_text](url) --> [link_text](url)
    html_table_pattern = re.compile(
        r"<Table.*?</Table>\s*", re.DOTALL | re.IGNORECASE
    )  # remove table HTML and its contents
    ...
    df[["module", "chapter", "section", "subsection"]] = df[
